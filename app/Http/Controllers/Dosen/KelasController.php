...
class KelasController extends Controller
{
    ...
    public function show($id)
    {
        ...
        foreach ($kelas->pertemuans as $pertemuan) {
            $this->syncPertemuanStatus($pertemuan);
        }
    ...
    public function storePertemuan(Request $request, $id)
    {
        $messages = [
            ...
            'tanggal.after_or_equal' => 'Gagal! Tanggal pertemuan tidak boleh hari kemarin.',
    ...
    public function togglePertemuan($id)
    {
        try {
            ...
            $tglPertemuan = Carbon::parse($p->tanggal)->format('Y-m-d');
            ...
            if ($tglPertemuan < $hariIni) {
                ...
                return back()->with('error', "Gagal! Pertemuan ini sudah melewati hari jadwal perkuliahan dan telah dikunci permanen.");
            }

            if ($tglPertemuan > $hariIni) {
                return back()->with('error', "Gagal! Sesi presensi belum bisa dibuka sebelum tanggal jadwal perkuliahan.");
            }

            if ($p->status !== 'dibuka') {
                $p->status = 'dibuka';
                $message = "Sesi presensi pertemuan ke-{$p->pertemuan_ke} BERHASIL DIBUKA MANUAL.";
            } else {
                $p->status = 'ditutup';
                $message = "Sesi presensi pertemuan ke-{$p->pertemuan_ke} BERHASIL DITUTUP MANUAL.";
            }
    ...
    {
        $pertemuanModel = Pertemuan::find($id);
        if ($pertemuanModel) {
            $this->syncPertemuanStatus($pertemuanModel);
        }
    ...
    private function syncPertemuanStatus($pertemuan)
    {
        $hariIni = Carbon::today()->format('Y-m-d');
        $tglPertemuan = Carbon::parse($pertemuan->tanggal)->format('Y-m-d');
        $waktuSekarang = Carbon::now();

        if ($tglPertemuan < $hariIni) {
            if ($pertemuan->status !== 'ditutup' || $pertemuan->is_manual) {
                $pertemuan->status = 'ditutup';
                $pertemuan->is_manual = false;
                $pertemuan->save();
            }
            return;
        }
        ...
        if ($tglPertemuan === $hariIni) {
            ...
            if ($pertemuan->is_manual) {
                return;
            }

            $jamMulai = Carbon::parse($tglPertemuan)->setTimeFromTimeString($pertemuan->jam_mulai);
            $jamSelesai = Carbon::parse($tglPertemuan)->setTimeFromTimeString($pertemuan->jam_selesai);

            if ($waktuSekarang->greaterThanOrEqualTo($jamMulai) && $waktuSekarang->lessThanOrEqualTo($jamSelesai)) {
                if ($pertemuan->status !== 'dibuka') {
                    $pertemuan->status = 'dibuka';
                    $pertemuan->save();
                }
            } elseif ($waktuSekarang->greaterThan($jamSelesai)) {
