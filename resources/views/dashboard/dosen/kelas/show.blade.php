            ...
            @if($next)
              <div
                ...
                <p class="text-xs text-slate-500 font-bold">
                  {{ \Carbon\Carbon::parse($next->tanggal)->translatedFormat('l, d M Y') }}
                </p>
              </div>
            @endif
          </div>
              ...
              <tbody>
                @foreach($kelas->pertemuans->sortBy('pertemuan_ke') as $p)
                    ...
                    <td class="px-6 py-4 text-right rounded-r-2xl">
                      <div class="flex justify-end gap-2">
                        ...
                        <a href="{{ route('dosen.pertemuan.show', $p->id) }}"
                          ...
                          title="Rekap Presensi Sesi #{{ $p->pertemuan_ke }}">
                        ...
                        <button data-micromodal-trigger="modal-edit-{{ $p->id }}"
                        ...
                        {{-- TOMBOL DELETE --}}
                        <button data-micromodal-trigger="modal-delete-{{ $p->id }}"
                  ...
                  <div class="modal" id="modal-edit-{{ $p->id }}" aria-hidden="true">
                    <div
                      class="modal__overlay fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-60 flex items-center justify-center"
                      tabindex="-1" data-micromodal-close>
                      <div
                        class="modal__container w-full max-w-md bg-white dark:bg-slate-800 rounded-[2.5rem] p-10 shadow-2xl mx-4"
                        ...
                        <div class="flex justify-between items-center mb-6">
                          <h2 class="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Edit Sesi
                            #{{ $p->pertemuan_ke }}</h2>
                          <button data-micromodal-close class="text-slate-400 hover:text-red-500 transition-colors"><i
                        ...
                        <form action="{{ route('dosen.pertemuan.update', $p->id) }}" method="POST"
                          ...
                          <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1">
                              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sesi
                                Ke-</label>
                            ...
                            <div class="space-y-1">
                              ...
                              <input type="date" name="tanggal" value="{{ $p->tanggal->format('Y-m-d') }}"
                                class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold outline-none focus:border-primary-500 transition-all"
                                required>
                            </div>
                          </div>
                          ...
                          <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1">
                              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Jam
                                Mulai</label>
                          ...
                          <div class="space-y-1">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lokasi
                              Presensi</label>
                            <select name="lokasi_id"
                              class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold focus:border-primary-500 transition-all">
                              @foreach($lokasis as $lok)
                                <option value="{{ $lok->id }}" {{ $p->lokasi_id == $lok->id ? 'selected' : '' }}>
                                  {{ $lok->nama }} ({{ $lok->radius_meter }}m)
                                </option>
                              @endforeach
                            </select>
                          </div>
                          ...
                          <div class="space-y-1">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Materi
                              Bahasan</label>
                          ...
                          <button type="submit"
                            class="w-full py-4 bg-amber-500 text-white rounded-2xl font-black shadow-xl shadow-amber-500/30 transform transition-all hover:-translate-y-1 mt-2">
                            SIMPAN PERUBAHAN
                          </button>
                        </form>
                      </div>
                  ...
                  <div class="modal" id="modal-delete-{{ $p->id }}" aria-hidden="true">
                    <div
                      class="modal__overlay fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-70 flex items-center justify-center"
                      tabindex="-1" data-micromodal-close>
                      <div
                        class="modal__container w-full max-w-sm bg-white dark:bg-slate-800 rounded-[2.5rem] p-10 shadow-2xl mx-4 text-center"
                        role="dialog" @click.stop>
                        <div
                        ...
                        <h2 class="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight mb-2">Hapus
                          Sesi?</h2>
                        <p class="text-sm text-slate-500 dark:text-slate-400 mb-8">
                          Data presensi mahasiswa pada pertemuan ke-{{ $p->pertemuan_ke }} akan <strong
                            class="text-rose-600">terhapus
                            permanen</strong>. Tindakan ini tidak bisa dibatalkan.
                        </p>

                        <div class="flex gap-3">
                          <button data-micromodal-close @click="MicroModal.close('modal-delete-{{ $p->id }}')"
                            class="flex-1 py-4 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-2xl font-black transition-all hover:bg-slate-200 uppercase text-xs tracking-widest">
                            Batal
                          </button>
                          <form action="{{ route('dosen.pertemuan.destroy', $p->id) }}" method="POST" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                              class="w-full py-4 bg-rose-600 text-white rounded-2xl font-black shadow-xl shadow-rose-500/30 transform transition-all hover:-translate-y-1 uppercase text-xs tracking-widest">
                              Ya, Hapus
                            </button>
                          </form>
                        </div>
