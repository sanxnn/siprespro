      ...
      <div class="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="flex items-center gap-4 sm:gap-5">
          ...
          <div class="min-w-0">
            ...
            <p class="text-[10px] sm:text-sm text-slate-500 font-bold flex flex-wrap items-center gap-1 sm:gap-2 uppercase tracking-widest mt-0.5 sm:mt-1">
        ...
        <table class="w-full text-left border-separate border-spacing-y-3">
          <thead>
            <tr>
              ...
              <th class="px-6 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Status Sesi</th>
            ...
            @foreach($kelas->pertemuans->sortBy('pertemuan_ke') as $p)
              <tr class="bg-slate-50 dark:bg-slate-900/50 hover:bg-white dark:hover:bg-slate-800 border border-slate-100 dark:border-slate-700 transition-all shadow-sm">
                ...
                <td class="px-6 py-4">
                  ...
                  <div class="flex flex-col gap-0.5 mt-1">
                    ...
                    <p class="text-[9px] text-slate-400 font-bold italic uppercase">
                      <i class="fas fa-clock mr-1"></i>{{ substr($p->jam_mulai, 0, 5) }} - {{ substr($p->jam_selesai, 0, 5) }}
                ...
                <td class="px-6 py-4 text-center">
                  ...
                  <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase {{ $status == 'Aktif' ? 'bg-emerald-100 text-emerald-600 animate-pulse' : 'bg-slate-200 text-slate-500' }}">
        ...
        @foreach($kelas->pertemuans->sortBy('pertemuan_ke') as $p)
          <div class="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs space-y-3">
            <div class="flex justify-between items-start gap-2">
              <div class="min-w-0">
                <span class="text-[10px] font-black px-2 py-0.5 bg-slate-200 dark:bg-slate-800 text-slate-500 rounded font-mono">SESI #{{ $p->pertemuan_ke }}</span>
                <h4 class="font-black text-slate-800 dark:text-white text-sm leading-tight mt-1.5 wrap-break-word">{{ $p->materi }}</h4>
              </div>
              <div class="shrink-0">
                @php $status = $p->status_absensi ?? ($p->status == 'dibuka' ? 'Aktif' : 'Ditutup'); @endphp
          ...
          <form action="{{ route('dosen.pertemuan.update', $p->id) }}" method="POST" class="space-y-4 text-left">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <input type="number" name="pertemuan_ke" value="{{ $p->pertemuan_ke }}" class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold outline-none focus:border-primary-500" required>
              </div>
              <div class="space-y-1">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <input type="time" name="jam_mulai" value="{{ substr($p->jam_mulai, 0, 5) }}" class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold focus:border-primary-500" required>
              </div>
              <div class="space-y-1">
            ...
            <div class="space-y-1">
              ...
              <select name="lokasi_id" class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold focus:border-primary-500">
                @foreach($lokasis as $lok)
                  <option value="{{ $lok->id }}" {{ $p->lokasi_id == $lok->id ? 'selected' : '' }}>{{ $lok->nama }} ({{ $lok->radius_meter }}m)</option>
                @endforeach
              </select>
            </div>
            <div class="space-y-1">
    ...
    <div class="modal__overlay fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4" tabindex="-1" data-micromodal-close>
      <div class="modal__container bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 sm:p-10 shadow-2xl w-full max-w-md relative" role="dialog" aria-modal="true">
        <div class="flex justify-between items-center mb-6">
          ...
          <button type="button" data-micromodal-close class="text-slate-400 hover:text-red-500"><i class="fas fa-times"></i></button>
        </div>
        <form action="{{ route('dosen.kelas.pertemuan.store', $kelas->id) }}" method="POST" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sesi Ke-</label>
              <input type="number" name="pertemuan_ke" value="{{ $kelas->pertemuans->count() + 1 }}" class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold outline-none focus:border-primary-500" required>
            </div>
            <div class="space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tanggal</label>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Jam Mulai</label>
              <input type="time" name="jam_mulai" class="w-full px-5 py-3 rounded-xl border-2 border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 font-bold focus:border-primary-500" required>
            </div>
            <div class="space-y-1">
              <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Jam Selesai</label>
