    ...
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">Monitoring kehadiran real-time berdasarkan
                    koordinat GPS</p>
            </div>
            ...
            <form action="{{ route('admin.presensi.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                ...
                <select name="pertemuan_id" onchange="this.form.submit()"
                    ...
                    @foreach($pertemuans as $p)
                        <option value="{{ $p->id }}" {{ request('pertemuan_id') == $p->id ? 'selected' : '' }}>
                            {{ $p->kelasPerkuliahan->mataKuliah->nama }} (P-{{ $p->pertemuan_ke }})
                        </option>
                    @endforeach
                </select>
                ...
                @if(request()->anyFilled(['pertemuan_id', 'status']))
                    <a href="{{ route('admin.presensi.index') }}"
                        class="text-xs font-bold text-rose-500 hover:underline">Reset</a>
                @endif
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700 text-left">
                            ...
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-all group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-900 flex items-center justify-center font-bold text-slate-500 dark:text-slate-400 text-xs border border-slate-200 dark:border-slate-700">
                                            {{ substr($presensi->mahasiswa->nama, 0, 1) }}
                                        </div>
                                        <div class="flex flex-col">
                                            <span
                                                class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ $presensi->mahasiswa->nama }}</span>
                                            <span
                                                class="text-[10px] text-slate-400 font-mono">{{ $presensi->mahasiswa->nim }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col items-center">
                                        <span
                                            class="text-xs font-bold text-slate-700 dark:text-slate-200">{{ date('H:i', strtotime($presensi->waktu_presensi)) }}</span>
                                        <span
                                            class="text-[9px] text-slate-400 uppercase font-black">{{ date('d M Y', strtotime($presensi->waktu_presensi)) }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase border {{ $colors[$presensi->status] }}">
                                        {{ $presensi->status }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <a href="https://www.google.com/maps?q={{ $presensi->latitude }},{{ $presensi->longitude }}"
                                        ...
                                        <span
                                            class="text-[10px] font-mono text-slate-500 dark:text-slate-400 truncate max-w-[120px]">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button @click="MicroModal.show('modal-edit-{{ $presensi->id }}')"
                                        ...
                                        <button @click="MicroModal.show('modal-delete-{{ $presensi->id }}')"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-20 text-center text-slate-400 italic">Data presensi belum ada!
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @foreach($presensis as $presensi)
        <div class="modal" id="modal-edit-{{ $presensi->id }}" aria-hidden="true">
            <div class="modal__overlay" tabindex="-1" data-micromodal-close>
                <div class="modal__container w-full max-w-sm bg-white dark:bg-slate-800 rounded-4xl p-8 shadow-2xl"
                    ...
                    <form action="{{ route('admin.presensi.update', $presensi->id) }}" method="POST">
                        ...
                        <div class="space-y-4 text-left">
                            <label
                                ...
                                Status Baru</label>
                            <select name="status"
                                class="w-full px-4 py-3 rounded-xl border dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-blue-500/50">
                        ...
                        <div class="mt-8 flex gap-3">
                            <button type="button" data-micromodal-close
                                class="flex-1 py-3 bg-slate-100 dark:bg-slate-700/50 text-slate-600 rounded-xl font-bold transition-all">Batal</button>
                            <button type="submit"
                                class="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-200">Update</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal" id="modal-delete-{{ $presensi->id }}" aria-hidden="true">
            <div class="modal__overlay" tabindex="-1" data-micromodal-close>
                <div class="modal__container w-full max-w-sm bg-white dark:bg-slate-800 rounded-4xl p-8 shadow-2xl"
                    role="dialog" @click.stop>
                    <div class="text-center">
                        <div
                            class="w-16 h-16 bg-rose-50 dark:bg-rose-900/30 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-4">
                            <i class="fas fa-trash-alt text-2xl"></i>
                        </div>
                        <h2 class="text-xl font-bold text-slate-800 dark:text-white">Hapus Data?</h2>
                        <p class="text-sm text-slate-500 mt-2">Data presensi <strong>{{ $presensi->mahasiswa->nama }}</strong>
                            akan dihapus permanen.</p>
                    </div>

                    <form action="{{ route('admin.presensi.destroy', $presensi->id) }}" method="POST" class="mt-8 flex gap-3">
                        @csrf @method('DELETE')
                        <button type="button" data-micromodal-close
                            class="flex-1 py-3 bg-slate-100 dark:bg-slate-700/50 text-slate-600 rounded-xl font-bold">Batal</button>
                        <button type="submit" class="flex-1 py-3 bg-rose-600 text-white rounded-xl font-bold">Hapus</button>
                    </form>
                </div>
            </div>
        </div>
