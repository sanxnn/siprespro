  ...
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-slate-500 dark:text-slate-400">Total {{ $semesters->total() }} periode akademik terdaftar
        </p>
      </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  @else
                    <form action="{{ route('admin.semester.set-aktif', $semester->id) }}" method="POST">
                      @csrf
                      <button type="submit"
                        class="text-[10px] font-bold text-slate-400 hover:text-primary-600 transition-colors uppercase flex items-center gap-1">
                        <i class="fas fa-power-off"></i> Set Aktif
                      </button>
                    </form>
                  @endif
                </td>
      ...
      <div class="modal__container w-full max-w-lg" role="dialog" @click.stop>
        <header class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100 dark:border-slate-700">
          <div>
            ...
            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-wider font-semibold">Pastikan
              format tahun ajaran benar</p>
          </div>
          <button
        ...
        <form action="{{ route('admin.semester.store') }}" method="POST">
          ...
          <div class="space-y-5">
            ...
            <div>
              <label
                class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-widest">Tahun
        ...
        <div class="modal__container w-full max-w-lg" role="dialog" @click.stop>
          <header class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100 dark:border-slate-700">
            <div>
              ...
              <p class="text-[10px] text-slate-400 mt-0.5 uppercase tracking-widest font-bold text-primary-500">Update data
                periode akademik</p>
            </div>
            <button
      ...
      <div class="modal__overlay" tabindex="-1" data-micromodal-close>
        <div class="modal__container w-full max-w-sm text-center" role="dialog" @click.stop>
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 mb-8 px-4 leading-relaxed text-center">
            Data <b>{{ $semester->nama }} {{ $semester->tahun_ajaran }}</b> akan dihapus permanen. Aksi ini tidak bisa
            dibatalkan!
          </p>
