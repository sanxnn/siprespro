    ...
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400">Total {{ $mahasiswas->total() }} mahasiswa terdaftar
                    dalam sistem</p>
            </div>
        ...
        <form method="GET" action="{{ route('admin.mahasiswa.index') }}"
            ...
            <input type="number" name="angkatan" value="{{ request('angkatan') }}" placeholder="Angkatan"
                class="w-28 px-4 py-2.5 text-sm rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-primary-500">

            <button type="submit"
                class="px-6 py-2.5 text-sm font-bold text-white bg-primary-600 hover:bg-primary-700 rounded-xl transition shadow-lg">
                Apply
            </button>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Profil Mahasiswa
                            </th>
                            ...
                            <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider text-slate-500">Aksi
                            </th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-all group">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/40 flex items-center justify-center text-primary-600 dark:text-primary-400 font-bold">
                                            {{ substr($mhs->nama, 0, 1) }}
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="text-sm font-semibold text-slate-700 dark:text-slate-200 block">{{ $mhs->nim }}</span>
                                    <span class="text-[10px] text-slate-400 block italic">NIK: {{ $mhs->nik ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-[10px] font-bold uppercase mb-1 border border-blue-100 dark:border-blue-800">
                                        {{ $mhs->golongan->nama ?? 'N/A' }}
                                    </span>
                                    <p class="text-xs text-slate-500 dark:text-slate-400">Angkatan {{ $mhs->angkatan }} |
                                        Semester {{ $mhs->semester_aktif ?? 'N/A' }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-xs text-slate-600 dark:text-slate-300"><i
                                            class="fas fa-phone text-[10px] mr-1 text-primary-500"></i> {{ $mhs->no_hp ?? '-' }}
                                    </p>
                                    <p class="text-[10px] text-slate-400 truncate max-w-[150px]"><i
                                            class="fas fa-map-marker-alt mr-1 text-red-400"></i> {{ $mhs->alamat ?? '-' }}</p>
                                </td>
            ...
            <div class="modal__container w-full max-w-3xl" role="dialog" @click.stop>
                <header class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100 dark:border-slate-700">
                    <div>
                        ...
                        <p
                            class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-wider font-semibold">
                ...
                <form action="{{ route('admin.mahasiswa.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">NIM <span
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Nama
                                    Lengkap <span class="text-red-500">*</span></label>
                                <input type="text" name="nama" required placeholder="Nama sesuai ijazah"
                                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-primary-500 outline-none transition-all">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Email
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label
                                        class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Angkatan</label>
                                ...
                                <div>
                                    <label
                                        class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Golongan</label>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">No.
                                    WhatsApp</label>
                                <input type="text" name="no_hp" placeholder="08xxxxxxxxxx"
                                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Tgl
                                    Lahir</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Alamat</label>
                    ...
                    <div class="mt-8 flex gap-3">
                        <button type="button" @click="MicroModal.close('modal-create-mhs')"
                ...
                <div class="modal__container w-full max-w-2xl" role="dialog" @click.stop>
                    <header class="flex justify-between items-center mb-6 pb-4 border-b border-slate-100 dark:border-slate-700">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 mt-0.5 uppercase tracking-widest font-bold">NIM:
                                {{ $mhs->nim }}</p>
                        </div>
                        <button
                            class="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700"
                            @click="MicroModal.close('modal-edit-{{ $mhs->id }}')">
                            <i class="fas fa-times"></i>
                        </button>
                    </header>
                    ...
                    <form action="{{ route('admin.mahasiswa.update', $mhs->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                            <div class="space-y-4">
                                <h4 class="text-[10px] font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400">
                                    Data Akademik</h4>
                                <div>
                                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Nama
                                        Lengkap</label>
                                    <input type="text" name="nama" value="{{ $mhs->nama }}" required
                                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 outline-none">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Email
                                        Akun</label>
                            ...
                            <div class="space-y-4">
                                <h4 class="text-[10px] font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400">
                                    Kontak & Alamat</h4>
                                <div>
                                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Nomor
                                        HP</label>
                                ...
                                <div>
                                    ...
                                    <textarea name="alamat" rows="2"
                                        class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 resize-none focus:ring-2 focus:ring-blue-500 outline-none">{{ $mhs->alamat }}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="mt-8 flex gap-3">
                            <button type="button" @click="MicroModal.close('modal-edit-{{ $mhs->id }}')"
                                class="flex-1 py-3 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold transition-colors">Batal</button>
                            <button type="submit"
                                class="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-200 transition-all dark:shadow-lg dark:shadow-slate-900">Update
                                Profil</button>
                        </div>
                    </form>
                </div>
        ...
        <div class="modal" id="modal-delete-{{ $mhs->id }}" aria-hidden="true">
            <div class="modal__overlay" tabindex="-1" @click="MicroModal.close('modal-delete-{{ $mhs->id }}')">
                <div class="modal__container w-full max-w-sm text-center" role="dialog" @click.stop>
                    <div
                        class="w-20 h-20 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mx-auto mb-5 animate-pulse border-4 border-red-50 dark:border-red-900/20">
                    ...
                    <form action="{{ route('admin.mahasiswa.destroy', $mhs->id) }}" method="POST" class="flex gap-3 px-2">
                        ...
                        <button type="submit"
                            class="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold shadow-lg shadow-red-200 transition-all dark:shadow-lg dark:shadow-slate-900">Ya,
                            Hapus</button>
                    </form>
                </div>
            </div>
